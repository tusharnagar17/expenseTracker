import { View, Text, SafeAreaView, TextInput, StyleSheet, Pressable, TouchableOpacity, ScrollView, Alert } from 'react-native'
...
export default function addItem() {
  ...
  const saveExpense = async (currentUser: FirebaseAuthTypes.User) => {
    ...
    if(amount && description && category && date) {
      // console.log({amount, description, category, date: date.getTime()})
      await db()
        .ref(`/users/${currentUser.uid}/sessions/${sessionId}`).set({
            amount,
          description,
          category,
          date: date.getTime()
  ...
  const submitBtnPressed = async () => {
    ...
    // console.log("console all data", {amount, description, category, date})
  }
  return (
  <ScrollView style={{margin: 10, padding: 20}}>
    ...
    <View style={{marginTop: 20}}>
      ...
      <TextInput style={styles.formInput}  value={amount} onChangeText={(text: string)=> setAmount(text)} />
      
      {/* Description */}
      <Text style={styles.formText}>Description</Text>
      <TextInput style={styles.formInput} placeholder='Description' value={description}  onChangeText={(text: string)=> setDescription(text)} />
